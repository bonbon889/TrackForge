...
module ApplicationConfiguration
  require 'ostruct'
  require 'yaml'  
  if File.exists?( File.join(RAILS_ROOT, 'config', "#{RAILS_ENV}_application.yml") )
    file = File.join(RAILS_ROOT, 'config', "#{RAILS_ENV}_application.yml")
    users_app_config = YAML.load_file file
  end
  default_app_config = YAML.load_file(File.join(RAILS_ROOT, 'app_engine', 'community_engine', 'engine_config', 'application.yml'))
