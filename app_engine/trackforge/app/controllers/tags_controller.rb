class TagsController < BaseController
  ...
  def show
    prm = params[:id].gsub(/%2f/,'/')
    @tag = Tag.find_by_name(prm)
    if @tag.nil? 
    ...
    if params[:type]
      ...
      case params[:type]
        when 'Track'
          @pages, @tracks = paginate :tracks, :conditions => cond.to_sql, :include => :tags, :per_page => 20
          @pages, @photos, @users, @clippings = [], [], [], []
        when 'Post'
      ...
      @tracks = Track.find_tagged_with(@tag.name, :limit => 10)
